...
class DoctorEditOfferScreen extends GetView<OfferEditController> {
  DoctorEditOfferScreen({super.key});

  final offerData = Get.arguments[0] as DoctorOffersData;

  @override
  Widget build(BuildContext context) {
    return GetBuilder<OfferEditController>(initState: (_) {
      controller.setEditOfferData(offerData);
    }, builder: (_) {
      return Scaffold(
        appBar: AppBar(
                          ...
                          Expanded(
                            child: _buildTimeContainer(
                              title: controller.endTime.value == null
                                  ? "End with"
                                  : controller
                                      .formatDate(controller.endTime.value!),
                              ontap: () async {
                                FocusScope.of(context).unfocus();
                                final Timestamp pickDate =
                                    await showModalBottomSheet<dynamic>(
